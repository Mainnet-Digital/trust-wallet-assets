...
async function findNetwork(chain) {
    try {
        const correspondingNetwork = await axios.get(
            `${URL}/api/networks?filters[trustWalletFolderName]=${chain}`
        );
...
async function findToken(token) {
    try {
        const correspondingToken = await axios.get(
            `${URL}/api/trust-wallet-tokens?filters[trustWalletRepoFolderName]=${token.token}&filters[address]=${token.id}`
        );
    ...
    for (t of tokens) {
        ...
        try {
            if (!token.success) {
                ...
                const fullToken = objects.tokens.filter(
                    (x) => x.id === token.id && x.chain === token.token
                )[0];
